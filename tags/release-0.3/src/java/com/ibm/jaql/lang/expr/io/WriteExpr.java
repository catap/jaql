 ...
 * 
 */
@JaqlFn(fnName = "write", minArgs = 3, maxArgs = 4)
public class WriteExpr extends MacroExpr
{
  /**
   * @param exprs
   */
  ...
  public Expr expand(Env env) throws Exception
  {
    ...
    NameValueBinding tField = new NameValueBinding(Adapter.TYPE_NAME, exprs[0]);
    NameValueBinding vField = new NameValueBinding(Adapter.LOCATION_NAME,
        exprs[1]);
    RecordExpr rec = new RecordExpr(new Expr[]{tField, vField});
    if (exprs.length == 4)
    {
