...
// not threadsafe
final class RecordSerializer extends BinaryBasicSerializer<JsonRecord> implements PerfSerializer<JsonRecord>
{
  // -- private variables -------------------------------------------------------------------------
  
  private RecordSchema schema;
  private PerfBinaryFullSerializer nameSerializer;
  private PerfBinaryFullSerializer additionalSerializer;
  ...
  private BufferedJsonRecord t;
  
  /** Stores information about a required or optional field. */
  private static class FieldInfo
  {
    RecordSchema.Field field;
    PerfBinaryFullSerializer serializer;
    int index; // index in requiredInfo or optionalInfo
    JsonString name;
    
    FieldInfo(RecordSchema.Field field, PerfBinaryFullSerializer serializer)
    {
      this.field = field;
  ...
  private void init()
  {
    ...
    t.ensureCapacity(noRequiredOptional);
    
    //JsonStringCache.activate();
    ...
    for (int posRequired=0, posOptional=0, pos=0; pos < noRequiredOptional; pos++)
    {
      ...
      FieldInfo k = new FieldInfo(field, new PerfBinaryFullSerializer(field.getSchema()));
      allInfo[pos] = k;
      if (field.isOptional()) {
    ...
    {
      additionalSerializer = new PerfBinaryFullSerializer(schema.getAdditionalSchema());
      nameSerializer = new PerfBinaryFullSerializer(SchemaFactory.stringSchema());
    }
  ...
  public JsonRecord read(DataInput in, JsonValue target) throws IOException
  {
    ...
    {
      // This is basically a empty record
      ...
      MyBitSet optionalBits) throws IOException
  {
    ...
    for (int i=0; i<noRequiredOptional; i++)
    {
      ...
      names[n] = info.name;
      values[n] = info.serializer.read(in, null);
      n++;
    }
    return n-offset;
  }
