...
public class Jaql implements CoreJaql {
	...
	public static void main(String args[]) throws Exception {
		...
		// System.exit(0); // possible jvm 1.6 work around for
	...
	public static void run(String filename, Reader reader,
			OutputAdapter outputAdapter, OutputAdapter logAdapter,
			boolean batchMode) throws Exception {
		Jaql engine = new Jaql(filename, reader);
		if (outputAdapter == null) {
			PrintStream out = new PrintStream(System.out, true, "UTF-8"); // TODO:
																			// use
																			// system
																			// encoding?
																			// have
			...
			engine.setJaqlPrinter(new IODescriptorPrinter(outputAdapter
					.getWriter()));
		}
		...
		if (logAdapter != null) {
			engine.setExceptionHandler(new JsonWriterExceptionHandler(
					logAdapter.getWriter()));
		}
	...
	protected String explainMode = System.getProperty("jaql.explain.mode"); // eventually
																			// more
																			// modes:
																			// jaql,
																			// graphical,
	...
	// * @throws Exception
	// */
	// public JsonIterator invokeFunctionIter(String varName, JsonValue[] args)
	// throws Exception
	// {
	 ...
	 * 
	 * If the next statement is an assignment, explain, or empty statement, it
	 * is processed and the next statement considered.
	 * 
	 * If there is trouble parsing or compiling the statement, handleError() is
	 * called to process the error. handleError() may rethrow the exception or
	 * it may simply log the error and prepareNext() move on to the next
	 * statement.
	 * 
	...
	public Expr prepareNext() throws Exception {
		...
		nextStmt: while (true) {
			...
			try {
				...
				// !(expr instanceof QueryExpr && expr.child(0) instanceof
				// RegisterAdapterExpr) && // HACK: if we don't register,
				// explain will change or bomb. This will go away with the
				// registry.
				...
				if (expr instanceof AssignExpr || expr instanceof QueryExpr
						&& expr.child(0) instanceof RegisterAdapterExpr) // HACK:
																			// if
																			// we
																			// don't
																			// register,
				...
				// expr instanceof QueryExpr && expr.child(0) instanceof
				// RegisterAdapterExpr ) // HACK: if we don't register, explain
				// will change or bomb. This will go away with the registry.
				// {
				// expr.eval(context);
				// if( explainOnly &&
				// expr instanceof QueryExpr && expr.child(0) instanceof
	...
	/**
	 * Evaluate the next query in the script and return an iterator over the
	...
	/**
	 * Evaluate the next query in the script and return the result. If there is
	 * no such query, return null (which is ambiguous with a query that returns
	 * null).
	 * 
	 * @return
	 * @throws Exception
	...
	/**
	 * Run the script and print any output to out. If an error is detected,
	 * handleError() is called to process it. handleError may rethrow the
	 * exception. It is safe to call run again to resume the script.
	 * 
	 * @return <tt>true</tt> if we hit the end of the script (otherwise
	 *         exception)
	 * @throws Exception
	 */
	...
	/**
	 * Parse a jaql query, if it is a function definition, return a JaqlFunction.
	 * fnName represents the called function's name, if fnName is not equal with 
	 * the function name declared in query sentence, abord with an exception.
	 */
	protected JaqlFunction parseFunction(String fnName) throws Exception {
		...
		if (!fnName.equals(name)) {
			throw new IllegalAccessException("No such function called " + fnName);
		}

		try {
			jqlFn = (DefineJaqlFunctionExpr) expr.children()[0];
		} catch (Exception exp) {
			throw new IllegalAccessException("Not a valid function call: " + fnName);
		}
	 ...
	 * 
	 */
	protected void registerFunction(String fnName, JaqlFunction fn,
			FunctionArgs args) throws Exception {
		if (args != null) {
