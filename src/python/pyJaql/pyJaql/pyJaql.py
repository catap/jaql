...
from jpype import *
import ConfigParser,json,sys,types,traceback,os
...
class Jaql:
	...
	def execute(self,*args):
		...
		try:
			...
			while it.moveNext():
				...
				if isinstance(jv,JPackage("com").ibm.jaql.json.type.JsonAtom):
					jv_str=jv.toString()				
					if isinstance(jv,JPackage("com").ibm.jaql.json.type.JsonString):
	...
	def execute_script(self,*args):
		...
		JFile=JPackage("java").io.File
		f=JFile(args[0])
		self.jaql.setInput(args[0],JPackage("java").io.FileReader(f))
		if len(args)==1:
			#do nothing
			pass
		else:
	...
	def evaluate(self,*args):
		...
		try:
			...
			return json.read(str(value.toString()))
		except Exception,err:
			raise JAQLRuntimeException(str(err))
	...
	def __setVar(self,varName,value):	
		...
		pkg=JPackage("com").ibm.jaql.json.type
