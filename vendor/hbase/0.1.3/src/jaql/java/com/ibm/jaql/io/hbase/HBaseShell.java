...
public class HBaseShell
{
      ...
      String[] vals) throws Exception
  {
    ...
    HTable m_table = new HTable((HBaseConfiguration) m_conf, new Text(table));
    long xid = m_table.startUpdate(new Text(row));
    for (int i = 0; i < cols.length; i++)
    {
      m_table.put(xid, new Text(cols[i]), vals[i].getBytes());
    }
    m_table.commit(xid);
    m_table.close();
  ...
  public String deleteRowVals(String args) throws Exception
  {
    ...
    HTable m_table = new HTable((HBaseConfiguration) m_conf, new Text(argv[0]));
    Text tKey = new Text(argv[1]);
    String[] cols = new String[argv.length - 2];
    for (int i = 0; i < cols.length; i++)
    ...
    for (int i = 0; i < cols.length; i++)
    {
      m_table.delete(xid, new Text(cols[i]));
    }
    m_table.commit(xid);
    m_table.close();
  ...
  private String printRow(String table, String key) throws Exception
  {
    ...
    HTable m_table = new HTable((HBaseConfiguration) m_conf, new Text(table));
    SortedMap<Text, byte[]> row = m_table.getRow(new Text(key));
    str.print(key + "=> ");
    for (Iterator<Text> iter = row.keySet().iterator(); iter.hasNext();)
    {
      Text label = iter.next();
      str.print("[" + label + "," + new String(row.get(label)).trim() + "]");
    }
    str.println();
    m_table.close();
      ...
      throws Exception
  {
    ...
    for (int i = 0; i < cols.length; i++)
    {
      byte[] val = m_table.get(tKey, new Text(cols[i]));
      if (val != null)
  ...
  {
    HTable m_table = new HTable((HBaseConfiguration) m_conf, new Text(table));
    Text[] colNames = new Text[cols.length];
    for (int i = 0; i < cols.length; i++)
    {
      colNames[i] = new Text(cols[i].trim());
    }

    ByteArrayOutputStream bstr = new ByteArrayOutputStream();
    PrintStream str = new PrintStream(bstr);
    HScannerInterface scanner = m_table.obtainScanner(colNames, new Text());
    HStoreKey key = new HStoreKey();
    TreeMap<Text, byte[]> current = new TreeMap<Text, byte[]>();
    while (scanner.next(key, current))
    {
      str.print(key.getRow() + "=>");
      for (Iterator<Text> iter = current.keySet().iterator(); iter.hasNext();)
      {
        Text vk = iter.next();
        str.print("[" + vk + "," + new String(current.get(vk)).trim() + "]");
      }
      str.println();
      current.clear();
    }
