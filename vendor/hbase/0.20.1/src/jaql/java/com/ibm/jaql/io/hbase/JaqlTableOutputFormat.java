 ...
 * 
 */
public class JaqlTableOutputFormat implements OutputFormat<Item, Item>
{
  ...
  protected class JaqlTableRecordWriter implements RecordWriter<Item, Item>
  {
    ...
    {
      m_table = table;
      LOG.info("JAQL_HBASE: set table: " + m_table);
    }
    ...
    {
      LOG.info("JAQL_HBASE: write table: " + m_table);
      ...
      JobConf job, String name, Progressable progress) throws IOException
  {
    ...
    try
    {
      for(int i = 0; i < num_tries && table == null; i++) {
        HBaseConfiguration hconf = new HBaseConfiguration();
        table = new HTable(hconf, tableName);
        if(table == null) {
          LOG.warn("table: " + tableName + " NULL");
          LOG.warn(hconf);
        }
      }
    }
    ...
    LOG.info("JAQL_HBASE: create writer: " + table);
