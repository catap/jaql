...
//      }
  ...
  private void groupToMapReduce(Segment groupSeg)
  {
    ...
    Expr mapFn = new CaptureExpr(new Var[]{valVar}, expr);    
    ...
    Expr aggFn = new CaptureExpr(new Var[]{keyVar,valVar}, expr);
    ...
    Expr finalFn = new CaptureExpr(new Var[]{keyVar,valVar}, lastExpr);
...
//    JsonSchema reduceOutputValueSchema = new JsonSchema(group.getSchema().elements()); 
    ...
    for (int i = 0; i < n; i++)
    {
      ...
      mapFns[i] = new CaptureExpr(new Var[]{inputState.mapIn}, forExpr);
    ...
    reduce = new CaptureExpr(reduceParams, reduce);
...
//    if (1==1) throw new IllegalStateException(expr.getSchema().toString());
    ...
    Expr mapFn = new CaptureExpr(new Var[]{mapIn}, expr);
  ...
  protected Segment segment(Expr expr)
  {
    ...
    else if (expr instanceof CaptureExpr)
    {
