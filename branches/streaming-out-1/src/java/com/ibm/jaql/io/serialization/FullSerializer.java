...
/**
 * Superclass for serializers of <code>JsonValue</code>s of unknown type.
 * <code>FullSerializer</code>s embed type information into their output. They
 * make use of {@link BasicSerializer}s to serialize the actual values. A
 * particular <code>FullSerializer</code> can only read values that have been
 * written by itself.
 * 
 * @param <In> type of input
 * @param <Out> type of output
 */
public abstract class FullSerializer<In, Out> implements
   ...
   * does not read more bytes than necessary to make the decision, and (3)
   * guarantees to read the entire encoded arrays in case of equality.
   * 
   ...
   * @throws IOException
   */
  public static <In> int compareArrays(In input1,
                                       int count1,
                                       In input2,
                                       int count2,
                                       FullSerializer<In, ?> serializer) throws IOException {
